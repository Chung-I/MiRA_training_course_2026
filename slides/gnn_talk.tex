...
\begin{frame}{Backup: why does accuracy collapse at 8 layers?}
  \vspace{-0.2em}
  ...
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.47\textwidth}
      \textbf{8 layers:} the baseline train accuracy drops to 88.1\%.
      ...
      71.7\%, because over-smoothing degrades generalization even when
      training succeeds.
    \end{column}
  \end{columns}
\end{frame}
...
The key finding: residuals fix the training failure at 8 layers but do not
fix the test accuracy loss. The model fits the data perfectly at every depth
with residuals, but generalizes worse with more layers. Over-smoothing is the
generalization bottleneck. Training difficulty is the training bottleneck.
Both contribute, at different depths.

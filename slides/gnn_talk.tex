...
over-smoothing layer sweep is in the appendix.
...
\begin{frame}{Backup: over-smoothing after 3 layers (GCN on Cora)}
  \vspace{0.2em}
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \centering\small
      ...
      \vspace{0.4em}
      {\scriptsize Layer sweep run with the PyG node-classification tutorial.\\
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{0.3em}
        ...
        \item On Cora, the optimum is \alert{2 layers}.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
\note{Backup. Numbers from the PyG node-classification tutorial on Cora
(GCN, hidden=16, 5 runs averaged). The MLP baseline (57.1\%) uses the same
node features without graph structure. The Colab link with our layer sweep
is on the PyG tutorials slide.

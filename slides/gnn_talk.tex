...
\begin{frame}{A GNN accepts a variable number of nodes and edges}
  \vspace{1.0em}
  A GNN represents its input as a set of nodes and an edge list. Adding or
  removing a node changes the set, not a fixed-size vector, so the same
  trained weights run on inputs of any size.
  \vspace{1.0em}
  \begin{itemize}
    \setlength{\itemsep}{0.7em}
    \item GNS trained on scenes with thousands of particles and generalized to
          $10\times$ more particles at test time. \arx{2002.09405}
    \item DPI-Net uses one node per particle, and that count differs per object
          and per timestep (fluids gain and lose particles). \arx{1810.01566}
    \item A scene graph has one node per detected object, and the number of
          objects in the room changes between scenes. \arx{2002.06289}
  \end{itemize}
\end{frame}
\note{The mechanism: message passing applies the same learned function to each
node and its neighbors, so the number of nodes is not a parameter of the model.
An MLP takes a fixed-length vector, so the number of objects is baked into the
architecture.}

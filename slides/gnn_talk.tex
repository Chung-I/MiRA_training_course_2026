...
\begin{frame}{Deeper is worse: over-smoothing after 3 layers}
  ...
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \centering\small
      \textbf{GCN on Cora} (2,708 nodes, 7 classes, hidden=16)
      \vspace{0.3em}
      ...
      \begin{tabular}{@{}rr@{}}
        ...
        \midrule
        1 & 80.7\% \\
        \textbf{2} & \textcolor{good}{\textbf{81.1\%}} \\
        3 & 77.9\% \\
        4 & 69.2\% \\
        6 & \textcolor{acc}{\textbf{49.2\%}} \\
        8 & \textcolor{acc}{\textbf{35.9\%}} \\
        \bottomrule
      \end{tabular}

      \vspace{0.4em}
      {\scriptsize Measured with the PyG node-classification tutorial.\\
       An MLP on the same features reaches 57.1\%.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \footnotesize
      ...
      \begin{itemize}\setlength{\itemsep}{0.3em}
        ...
        \item On Cora, the optimum is \alert{2 layers}. Deeper GCNs rarely
          help unless the graph has been rewired (see over-squashing).
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
\note{Numbers measured by running the official PyG node-classification tutorial
on Cora with GCN (hidden=16, 5 runs averaged). The MLP baseline (57.1\%) uses
the same node features without any graph structure.

The live demo: open the tutorial, change num\_layers, and re-run. Accuracy
visibly drops at 6 and 8 layers.

Li et al., ``Deeper Insights into Graph Convolutional Networks for
Semi-Supervised Learning,'' AAAI 2018.

If you need long-range information, add a global node or a hierarchy. Do not
add depth.}
...
\begin{frame}{PyTorch Geometric tutorials on Colab}
  ...
  \begin{tabular}{@{}llrl@{}}
    \toprule
    \textbf{Tutorial} & \textbf{Dataset} & \textbf{Result} & \textbf{Graph} \\
    \midrule
    \href{https://colab.research.google.com/drive/1h3-vJGRVloF5zStxL5I0rSy4ZUPNsjy8}{Introduction}
      & KarateClub (34 nodes)
      & 76.5\%
      & social network \\[0.3em]
    \href{https://colab.research.google.com/drive/14OvFnAXggxB8vM4e8vSURUp1TaKnovzX}{Node classif.}
      & Cora (2,708 nodes)
      & \textcolor{good}{\textbf{81.1\%}}
      & citation \\
      & {\color{dim}MLP baseline}
      & {\color{dim}57.1\%}
      & {\color{dim}(no graph)} \\[0.3em]
    \href{https://colab.research.google.com/drive/1I8a0DfQ3fI7Njc62__mVXUlcAleUclnb}{Graph classif.}
      & MUTAG (188 graphs)
      & 78.9\%
      & molecular bonds \\[0.3em]
    \href{https://colab.research.google.com/drive/1D45E5bUK3gQ40YpZo65ozs7hg5l-eo_U}{Point clouds}
      & ModelNet10
      &
      & radius ball \\
    \bottomrule
  ...
  \begin{center}\small
    The 24-point gap between MLP and GCN on Cora comes from the citation
    graph. The node features alone do not separate the classes.
  \end{center}
\end{frame}
\note{All four run on a free Colab CPU or T4. Each installs PyG inside the
notebook.

The node-classification result (MLP 57.1\% vs GCN 81.1\%) is the single most
useful number in the talk for explaining why graph structure matters. The
over-smoothing table on the previous slide comes from the same tutorial with
the layer count varied.

...
% --- 9 ----------------------------------------------------------------
\begin{frame}{CNN, Transformer, GNN: three assumptions about structure}
  \vspace{0.2em}
  \begin{center}
    \begin{tikzpicture}[
      font=\scriptsize,
...
\begin{frame}{Backup: over-smoothing after 3 layers (GCN on Cora)}
  ...
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tabular}{@{}rr@{}}
        ...
        \midrule
        1 & 75.1\% \\
        \textbf{2} & \textcolor{good}{\textbf{80.7\%}} \\
        3 & 78.0\% \\
        4 & 76.8\% \\
        6 & \textcolor{acc}{\textbf{74.4\%}} \\
        8 & \textcolor{acc}{\textbf{61.6\%}} \\
        \bottomrule
      \end{tabular}

      \vspace{0.4em}
      {\scriptsize From the node-classification notebook (5 seeds each).\\
       An MLP on the same features reaches 54.7\%.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \footnotesize
  ...
  \end{columns}
\end{frame}
\note{Backup. Numbers from \texttt{notebooks/pyg\_node\_classification.ipynb}
(GCN on Cora, hidden=16, 5 seeds averaged). The MLP baseline (54.7\%) uses
the same features without graph structure. Open the notebook in Colab to see
the cached outputs and re-run.

...
\begin{frame}{Four things occupy VRAM, and the KV cache is the one most people forget}
  ...
  \begin{itemize}
    ...
    \item \textbf{Activations}: small at inference, and batch-dependent.
    \item \textbf{Overhead / fragmentation}: allocator, CUDA context, workspace.
          A 2\,GB budget covers it.
  \end{itemize}
  \vspace{0.6em}
  \uncover<2->{Weights are a fixed cost. The KV cache grows with each concurrent user.}
\end{frame}
...
\begin{frame}{GQA groups query heads to share fewer key-value pairs}
  ...
  \vspace{0.4em}
  \begin{itemize}\small
    \item \textbf{MHA} gives each query head its own key-value head.
          Cache size $\propto$ number of heads.
    \item \textbf{GQA} groups $G$ query heads to share one KV head.
          Llama-3-8B uses $G{=}4$: 32 query heads, 8 KV heads.
    \item \textbf{MQA} is the extreme: all query heads share one KV head.
          Faster, but quality degrades on long output.
  \end{itemize}
  \vspace{0.3em}
  \begin{center}\small
  ...
  \end{center}
\end{frame}
\note{\textbf{8:00.} The diagram is the content. Point at the arrows. GQA-8
means 8 groups, which for Llama-3 means 32/8 = 4 query heads per KV head.
The uptraining result matters: you can convert an existing MHA checkpoint to
GQA by mean-pooling the KV projection matrices, then training for 5\% more
steps.
...
\begin{frame}{\texttt{generate()} reaches 9.4\% of the hardware}
  ...
  \end{center}
  \vspace{0.4em}
  \emphc{vLLM or SGLang closes most of that 90\% gap with continuous
...
% -------------------------------------------------------------------------
\begin{frame}{Backup: NVFP4 on Blackwell halves FP8 memory again}
  \small
  NVFP4 is a 4-bit floating-point format: 1 sign bit, 2 exponent bits, 1
  mantissa bit. It stores values from $-6$ to $+6$.
  \vspace{0.4em}
  \begin{center}
    \begin{tabular}{lrrr}
      \toprule
  ...
  \vspace{0.4em}
  \begin{itemize}\small
    \item NVFP4 uses \textbf{two-level scaling}: each block of 16 values shares
          an E4M3 scale factor, and the whole tensor has an FP32 scale. The
          finer block size and floating-point (not power-of-two) scale preserve
          more dynamic range than MXFP4.
  ...
  \end{itemize}
\end{frame}
\note{Backup. Use if someone asks about FP4 or Blackwell quantization. The
two-level scaling is the key: MXFP4 uses a coarser block (32 values) with a
power-of-two scale, which loses more precision on outlier-heavy weight
distributions. NVFP4 keeps a floating-point scale on a 16-value block.
...
\begin{frame}{\cutbadge{2: trim next} \; Multi-Token Prediction trains the draft head into the model}
  \textbf{MTP} trains the model to predict $D$ tokens ahead, not just the
  next one. At inference the extra heads serve as a \emph{built-in draft}
  ...
  \begin{center}\small
    ...
    (+6.1 points) on the small MoE, and from 44.5\% to 53.7\% (+9.2) on the
    large MoE. The improvement comes from training, not from inference.
  \end{center}
\end{frame}
\note{MTP is both a training technique and an inference technique. The training
effect is the main one: the auxiliary heads act as a regularizer and improve
quality even when discarded at inference. The inference effect (speculative
decoding with no separate draft model) is a bonus.

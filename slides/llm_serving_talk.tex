...
\begin{frame}{\#2--3 Quantize the weights, then quantize the cache}
  ...
  \end{center}
\end{frame}
\note{\textbf{21:30.} Do not survey quantization algorithms. Name AWQ, GPTQ and
FP8, give the rule, and continue. If someone asks which one, answer: the one
that your engine supports for your card.}
...
\begin{frame}{The SGLang trick is RadixAttention: longest-prefix match}
  \hfill\llmdiagKV
  \textbf{RadixAttention has the same goal as prefix caching, with a different
  data structure.}
  \vspace{0.6em}
  \begin{center}
    \begin{tabular}{ll}
  ...
  \begin{itemize}
    ...
    \item That structure also makes the cache \textbf{tierable}:
          you can evict a subtree instead of a guess about blocks.
  \end{itemize}
  \vspace{0.5em}
  \emphc{If your workload is many requests that share long, varied prefixes
  (agents, tool calls, few-shot prompts), this is the difference.}
\end{frame}
\note{\textbf{25:00.} The distinction that matters: a hash needs an exact prefix match at a
block boundary. A radix tree finds partial overlap. Agent
...
\begin{frame}{\cutbadge{1: cut this first} \; The layer below: Triton, and a training contrast}
  ...
  \begin{itemize}
    ...
    \item \textbf{Liger-Kernel} is a set of fused Triton kernels (RMSNorm, RoPE,
          SwiGLU, fused linear cross-entropy). It claims $+20\%$ throughput and
          $-60\%$ memory.

          \vspace{0.3em}
          \warnc{Liger-Kernel is for training, not serving.} It is the same
          technique applied to the other half of the lifecycle.
  \end{itemize}
\end{frame}
\note{This whole frame is Movement 3d. If the talk runs long, remove this
frame first. One optional slide, and the first to cut. If you are past 30 minutes
when you reach Movement 3c, skip this.}
...
\begin{frame}{\cutbadge{2: trim next} \; \#5 Speculative decoding: 2$\times$, and the same output}
  \begin{itemize}
    ...
    \item The model keeps the accepted tokens. The first rejection restarts from there.
  \end{itemize}
  \vspace{0.8em}
  \begin{center}
    \begin{minipage}{0.9\textwidth}
      \begin{block}{Why it works, a callback to Movement 2}
        ...
        Verification of $k$ tokens costs about the weight traffic of one
        token. You buy speed with compute that you did not use.
      \end{block}
    \end{minipage}
  \end{center}
  \vspace{0.6em}
  Usually 1.5--3$\times$ faster, with an \emphc{identical output
  distribution}.
\end{frame}
\note{The identical distribution sets it apart from the other tricks: there
is a proof of zero quality loss. If you have 30 seconds, use them here.}

...
\begin{frame}{Batching is nearly free: 15$\times$ the throughput, 4\% slower per user}
  ...
  \begin{itemize}
    ...
    \item And what limits the batch? \emphc{The KV cache}, which takes us back to Movement 1.
  \end{itemize}
\end{frame}
...
\begin{frame}{SGLang's trick is RadixAttention: longest prefix, not a hash hit}
  ...
  \begin{itemize}
    \item A tree of \emph{spans} finds the longest shared prefix between a new
          request and everything already cached, rather than only an exact hit.
...
\begin{frame}{TensorRT-LLM's trick is specializing to your exact hardware}
  ...
  \begin{center}
    \begin{minipage}{0.92\textwidth}
      \begin{alertblock}{Two things we found by actually running it}
        \textbf{1.} Version 1.2.1 defaults to a \textbf{PyTorch backend}, not the
        engine build. The compile step is now a path you opt into.

        \vspace{0.3em}
        \textbf{2.} When you do compile, you compile \textbf{per model, per shape
        range, per GPU}, and rebuild when any of the three changes. That is the
        trade: the last of the performance, for a build step in your pipeline.
      \end{alertblock}
    \end{minipage}
  \end{center}
\end{frame}
\note{\textbf{27:00.} The fundamental tradeoff of the section: specialization
...
checked, and 1.2.1 starts a PyTorch runtime by default. The older framing is
still repeated everywhere online and is now out of date.}
...
\begin{frame}{We measured all three. An engine is worth about 4$\times$.}
  ...
  \vspace{0.5em}
  \begin{itemize}\small
    \item \emphc{Not a batching artifact.} \texttt{generate()} batches fine and
          all of them scale nearly linearly. The engines are simply
    ...
    \item \textbf{Do not read a winner here.} On a 0.5\,B model this is dispatch
          overhead, not architecture, and TRT-LLM is far outside its design
          envelope.
  \end{itemize}
\end{frame}
...
measuring per-request overhead, not the engines.}

% -------------------------------------------------------------------------
\begin{frame}{The default that quietly costs you half your throughput}
  ...
  \begin{center}
    \begin{minipage}{0.93\textwidth}
      \begin{alertblock}{The lesson, and it is the one to remember}
        The missing CUDA toolkit announced itself with a stack trace. This one
        produced a \textbf{plausible, quotable, completely wrong} throughput
        curve. \emphc{Benchmark at the concurrency you will actually serve.}
      \end{alertblock}
    \end{minipage}
  \end{center}
\end{frame}
\note{\textbf{30:30.} The best slide in this section. Measured only to 16 and the
...
\begin{frame}{FlashRT: no compile, hand-written kernels, one CUDA graph}
  ...
  \begin{itemize}
    ...
    \item \textbf{The entire forward pass captured as one static CUDA graph}
          so there is zero Python dispatch at replay time.
...
first, then the tail of the toolbox. Movements 3b and 3c are never cut, because
...
\begin{frame}{Backup: none of the three ran from \texttt{pip} on a clean machine}
  ...
  \vspace{0.3em}
  \begin{center}
    \emphc{This is the argument for running them in their official containers.}
  \end{center}
\end{frame}
...
\begin{frame}{Backup: five TensorRT-LLM configs, none of them helped}
  ...
  \end{center}
  \vspace{0.5em}
  What is left is not configurable: a 0.5\,B model emitting 128 tokens is
  dominated by \textbf{per-request overhead}, not by the kernels TensorRT-LLM
  optimizes.
  \vspace{0.3em}
  \begin{center}
  ...
  \end{center}
\end{frame}
\note{Backup. The honest negative result. Also the answer to "why is TRT-LLM
last in your table" -- because we benchmarked it on the wrong thing, deliberately
kept in to make that point.}
...
\begin{frame}{Backup: the questions that always come}
  ...
  \begin{itemize}
    ...
    \item \textbf{Does quantization hurt quality?} Sometimes, and not
          uniformly across tasks. 4-bit weight-only is usually fine, but confirm

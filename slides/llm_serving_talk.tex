...
\begin{frame}{Four things occupy VRAM, and the KV cache is the one most people forget}
  ...
  \end{itemize}
  \vspace{0.6em}
  \uncover<2->{The weights and the KV cache set the budget.}
\end{frame}
...
\begin{frame}{The KV cache formula, which most people skip}
  ...
  \end{itemize}
  \vspace{0.6em}
  \emphc{Four numbers, all in the \texttt{config.json} of the model.}
\end{frame}
...
\begin{frame}{\texttt{generate()} reaches 9.4\% of the hardware}
  ...
  \end{center}
  \vspace{0.4em}
  \emphc{A serving engine exists to close that 90\% gap.}
\end{frame}
...
\begin{frame}{We measured all three. An engine is worth about 4$\times$.}
  ...
  \begin{itemize}\small
    ...
    \item \textbf{This table does not rank the engines.} On a 0.5\,B model the
          measurement is dominated by dispatch overhead, and TRT-LLM is far
